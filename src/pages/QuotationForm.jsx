...
export function QuotationFrom() {
  ...
  const fetchUserRole=async ()=>{
    ...
    // Function to get quotation data
    const quotationCall = async () => {
      try {
    ...
    // Execute all the API calls concurrently
    const fetchData = async () => {
      await Promise.all([quotationCall(), getBussinessProfile(), fetchProduct(), fetchCustomers()]);
    };
  
    // Run the fetchData function to initiate all API calls
    fetchData().finally(() => {
  ...
  const formik = useFormik({
    ...
    onSubmit: (values) => {
      ...
      values.due_date = quotationDue;
      values.quotation_prefix = quotationPrefix;
      ...
      const postQuotation = async () => {
        ...
        try {
          const res = await apiPost("/quotation", values);
          navigate("/quotation");
        } catch (error) {
          console.error("Error posting quotation:", error);
        } finally {
          setLoader(false); // 🔴 Stop loader whether success or failure
        }
      };
      postQuotation();
    },
  ...
  //   const updateDescription = [...productDescription];
  //   updateDescription[index] = e.target.value;
  //   setProductDescription(updateDescription);
  ...
  //       for (let row of updatedRows) {
  //         totalAmount += parseFloat(row.total);
  //         taxableamount += (parseFloat(row.taxable_amount)).toFixed(2);
  //       }
  //       for (var j = 0; j < updatedRows.length; j++) {
  //         if (updatedRows[j].gstRate == term[1]) {
  //           gstSumPer += 1;
        ...
        <form onSubmit={formik.handleSubmit}>
          ...
          <div className="overflow-x-auto">
            ...
            {/* <ProductTable
             ...
             onBlurHandle={handleChange}
             onChanges={handleChange}
             onDelete={handleDeleteRow}
             TaxAmount={TaxAmount}
             onDescriptionChange={handleDescriptionAdd}
             ></ProductTable> */}
