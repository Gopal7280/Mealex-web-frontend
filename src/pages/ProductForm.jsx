    ...
    console.log(gstSet);
        if(name=="selling")
        {
          if(e.target.value=="withGst")
          {
            ...
            setGstSet({
              sellingPrice:"withGstSelling",
              purchasePrice:gstSet.purchasePrice!=undefined?gstSet.purchasePrice:"withGstPurchase",
            })
          }
        ...
        if(name=="purchase")
          {
            if(e.target.value=="withGst")
            {
              console.log(e.target.value);
              setGstSet({
                sellingPrice:gstSet.sellingPrice!=undefined?gstSet.sellingPrice:"withGstSelling",
                purchasePrice:"withGstPurchase",
              })
            }
            if(e.target.value=="withoutGst")
        ...
        <Toast ref={toast} />
          ...
          >
            <div className="md:grid md:grid-cols-2 md:gap-6 inline-block">
              ...
              <div className="">
              <label className="block mt-2 text-gray-600">Product Unit</label>
              <select onChange={(e) => handleChangeUnit(e)} className="w-full  p-2 border rounded">
                    {
                      unitSelected.length>0?(
                        unitSelected.map((item)=>(
                          <option value={item.unit_name}>{item.unit_name}</option>
                        ))
                      ):(<option value="no load">Api load failed</option>)
                    }
                </select>
              </div>
              <div>
                <label class="block mt-2 text-gray-600">Product Category</label>
              ...
              <div>
                <label class="block text-gray-600">Select with/without gst</label>
                <select onChange={(e)=>handleGstSelect(e,"selling")} class="w-full p-2 border rounded">
              ...
              <div>
                <label class="block text-gray-600">Select with/without gst</label>
                <select onChange={(e)=>handleGstSelect(e,"purchase")} class="w-full p-2 border rounded">
                  <option value="withGst">With GST</option>
                  <option value="withoutGst">Without GST</option>
                </select>
              </div>
              ...
              <div>
              <label class="block mt-2 text-gray-600">GST Category</label>
              <select onChange={(e) => handleChange(e)} class="w-full p-2 border rounded">
              ...
              <div>
                <button disabled className="mt-2 px-4 py-2 bg-blue-500 disabled:opacity-80 disabled:bg-gray-400 text-white rounded">Generate Barcode</button>
              </div>
              <div className="mt-2">
                <InputComponent
