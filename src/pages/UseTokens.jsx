...
// 1️⃣ Try to get charges from navigation state
const dineChargeState = location.state?.dineCharge;
const takeAwayChargeState = location.state?.takeAwayCharge;
const deliveryChargeState = location.state?.deliveryCharge;

// 2️⃣ Try to get from localStorage (stored in ActivePlanCard)
const storedCharges = storage.getItem("messCharges")
  ? JSON.parse(storage.getItem("messCharges"))
  : {};

// 3️⃣ Final unified charges (fallback handled)
const dineCharge = dineChargeState ?? storedCharges.dineCharge ?? 0;
const takeAwayCharge = takeAwayChargeState ?? storedCharges.takeAwayCharge ?? 0;
const deliveryCharge = deliveryChargeState ?? storedCharges.deliveryCharge ?? 0;
...
{/* 💰 Show Selected Service Charge */}
{orderType && (
  <div className="bg-orange-50 border border-orange-200 rounded-lg p-3 mb-4 text-center shadow-sm transition-all duration-200">
