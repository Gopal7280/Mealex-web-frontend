...
<div className="bg-white border border-gray-200 rounded-xl mb-8 p-4 shadow-sm">
  <div className="flex justify-between items-center mb-4">
    ...
    {expiringPlans.length > 0 && (
      <button
        onClick={() => navigate('/customer-activeplans')}
        className="text-sm text-orange-500 hover:text-orange-600 cursor-pointer font-medium transition"
      >
        View All →
      </button>
    )}
...
//       );
//       if (['accepted', 'rejected', 'cancelled'].includes(normalizedStatus)) {
//         toast.success(t(`orderStatus.${normalizedStatus}`));
//       }
//     };
...
//     socket.emit('cancel_order', orderPayload, (response) => {
//       if (response?.success) toast.success(t('orderCancelled'));
//       else toast.error(t('orderCancelFailed'));
//     });
//   };
...
//                 {totalMessSubscribed}
//               </h3>
//               <p className="text-sm text-gray-600 mt-1">{t('totalMessSubscribed')}</p>
//             </div>
//           </div>
...
//                 {totalActivePlans}
//               </h3>
//               <p className="text-sm text-gray-600 mt-1">{t('totalActivePlans')}</p>
//             </div>
//           </div>
//         </div>
...
//         <div className="bg-white border border-gray-200 rounded-xl overflow-hidden mb-8">
//           <div className="flex justify-between items-center px-4 sm:px-6 md:px-12 py-4">
//             <h3 className="text-base font-semibold text-[#33363F]">{t('recentOrders')}</h3>
//           </div>

//           {orders.length === 0 ? (
//             <div className="text-center text-[#0000008C] py-6">{t('noOrdersPlaced')}</div>
//           ) : (
//             <div className="overflow-x-auto">
//               <div className="min-w-[900px]">
//                 <div className="hidden md:grid grid-cols-7 text-sm font-medium text-gray-600 border-b border-gray-100 pb-2 px-4">
...
//                     >
//                       <div className="md:hidden space-y-2">
//                         <p><span className="font-medium">{t('customer')}:</span> {order.customerName}</p>
//                         <p><span className="font-medium">{t('plan')}:</span> {order.customerPlanName}</p>
//                         <p><span className="font-medium">{t('tokens')}:</span> {order.tokenCount}</p>
//                         <p><span className="font-medium">{t('type')}:</span> {order.orderType}</p>
//                         <p>
...
//         <div className="bg-white border border-gray-200 rounded-xl mb-8 p-4 shadow-sm">
//           <div className="flex justify-between items-center mb-4">
//             <h3 className="text-lg font-semibold text-gray-800">{t('expiringPlans')}</h3>
//             {expiringPlans.length > 0 && (
//               <button
//                 onClick={() => navigate('/customer-activeplans')}
//                 className="text-sm text-orange-500 hover:text-orange-600 cursor-pointer font-medium transition"
...
//                       <div className="mt-3 flex justify-between items-center text-xs font-medium">
//                         <span className="text-orange-500">
//                           {t('tokens')}: {plan.usedTokenCount}/{plan.issuedTokenCount}
//                         </span>
//                         <span className="text-green-600 font-semibold">₹{plan.price}</span>
//                       </div>
//                     </div>
