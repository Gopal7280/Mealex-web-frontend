  ...
  const formik = useFormik({
    ...
    onSubmit: values => {
      ...
      if (
        ...
        values.anniversary_date == undefined
      ) {
  ...
  useEffect(() => {
    ...
    const getState = async () => {
      try {
        ...
        setState(sortedStates); // Setting the state data for billing address
        setState1(sortedStates); // Setting the state data for shipping address
        setIso2Country('In'); // Setting the ISO2 code for billing address
        setIso2Country1('In'); // Setting the ISO2 code for shipping address
        setNames({
          countryName: 'India',
          stateName:
            location.state.data.billing_state == null
              ? 'Select'
              : location.state.data.billing_state,
          cityName:
            location.state.data.billing_city == null
              ? 'Select'
              : location.state.data.billing_city,
        }); // Setting the names for billing address
        setNames1({
          countryName: 'India',
          stateName:
            location.state.data.shipping_state == null
              ? 'Select'
              : location.state.data.shipping_state,
          cityName:
            location.state.data.shipping_state == null
              ? 'Select'
              : location.state.data.shipping_state,
    ...
    if (name == 'pan') {
      ...
      if (!panRegex.test(e.target.value.toUpperCase())) {
        if (e.target.value == '') {
          ...
          formik.values.pan_no = e.target.value.toUpperCase();
          setStatus(false);
        }
      } else {
        formik.values.pan_no = e.target.value.toUpperCase();
    ...
    if (name == 'number') {
      ...
      if (!panRegex.test(e.target.value)) {
        if (e.target.value == '') {
          ...
          formik.values.mobile_no = e.target.value;
          setStatus(false);
        }
      } else {
        formik.values.mobile_no = e.target.value;
    ...
    if (name == 'gstin') {
      ...
      if (!gstRegex.test(e.target.value.toUpperCase())) {
        if (e.target.value == '') {
          ...
          formik.values.tax_id = e.target.value.toUpperCase();
          setStatus(false);
        }
      } else {
        formik.values.tax_id = e.target.value.toUpperCase();
              ...
              <div className="grid grid-cols-2 gap-6">
                <div>
                  <InputComponent
                    ...
                    {...(isEdit
                      ? { type: 'date' }
                      : { value: data.customer_birthdate, type: 'date' })}
                  ></InputComponent>
                </div>
                <div>
                  <InputComponent
                    ...
                    {...(isEdit
                      ? { type: 'date' }
                      : { value: data.customer_anniversary, type: 'date' })}
                  ></InputComponent>
                </div>
              </div>
              <div>
